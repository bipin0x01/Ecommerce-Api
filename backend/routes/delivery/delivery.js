...
let deliveryModel = databaseEcommerce.createSchemaModel(
...
router.get("/get-delivery", AuthToken.jwtAuthentication, async (req, res) => {
  try {
    if (req.user.role == "admin") {
      let deliveryData = await databaseEcommerce.fetchDatabase(
      ...
      let deliveryData = await databaseEcommerce.fetchDatabase(
        { deliveryBoy: req.user.username, deliveryStatus: "pending" },
        {},
        deliveryModel
      );
      res.json({ status: "success", message: deliveryData.data });
      ...
      res.json({
        status: "failure",
        message: "Can only  be accessed by admin and delivery boy",
      });
    }
  } catch (err) {
...
router.post(
  ...
  async (req, res) => {
    if (req.user.role !== "admin" && req.user.role !== "delivery") {
      res.json({
        status: "faliure",
        message: "only accessed by admin or deliery",
      });
      return;
    }
    try {
      deliveryData = await databaseEcommerce.fetchDatabase(
        { orderId: req.body.id },
        { deliveryStatus: 1, deliveryBoy: 1, price: 1 },
        deliveryModel
      );
      ...
      databaseEcommerce.updateToModel(
        { orderId: req.body.id },
        {
          $set: {
            ...
            pricePaid: deliveryData.data[0].price,
            paid: true,
          },
        },
        orderModel
      ...
      databaseEcommerce.updateToModel(
        ...
        { $inc: { cashCollected: deliveryData.data[0].price } },
        staffModel
      );
      res.json({
        status: "success",
        message: "order completed please pay-cash to admin",
      });
    } catch (err) {
...
router.post("/submit-cash", AuthToken.jwtAuthentication, async (req, res) => {
  ...
  try {
    let userValid = await databaseEcommerce.fetchDatabase(
      { username: staffUsername },
      { cashCollected: 1 },
      staffModel
    );
    if (userValid.data[0].cashAmount != cashAmount)
      return res.json({ status: "failure", message: "Amount not matched" });
    await databaseEcommerce.updateToModel(
      { username: staffUsername },
      { $inc: { cashCollected: -cashAmount } },
      staffModel
    );
    databaseEcommerce.saveToModel({ cashed: cashAmount }, shopAccountModel);
    databaseEcommerce.saveToModel(
      { staffName: staffUsername, cashed: cashAmount },
      staffsAccountModel
    );
  } catch (err) {
    res.json({ status: "failure", message: err.data });
